# Codigo retirado de : https://github.com/ScrapCodes/image-recognition-tensorflow
# O dataset utilizado foi gerado a partir do mosaico de daninhas e foi baseado no
# dataset do chihuahua e do muffin
...
class_names = ['daninha', 'sem_daninha']
##
	...
	for file in glob.glob("*.jpg"):
		...
		if re.match('daninha_.*', file):
			...
			if invert_image:
				...
				labels.append(0)
				labels.append(0)
		elif re.match('sem_daninha_.*', file):
			images.append(img.reshape(reshape_size))
			labels.append(1)
			if invert_image:
				images.append(inv_image.reshape(reshape_size))
...
(train_images, train_labels) = load_image_dataset(
	path_dir='../../sensix_daninhas/dataset_resumed',
	maxsize=maxsize,
	reshape_size=(maxsize_w, maxsize_h, 1),
	invert_image=False)

(test_images, test_labels) = load_image_dataset(
	path_dir='../../sensix_daninhas/dataset_resumed/validation',
	maxsize=maxsize,
	reshape_size=(maxsize_w, maxsize_h, 1),
	invert_image=False)
